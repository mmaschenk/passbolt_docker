...
install() {
  ...
  if [ ! -f "$app_config" ]; then
    wwwdata-bash -c 'cp /var/www/passbolt/config/app.default.php /var/www/passbolt/config/app.php'
  fi
  ...
  wwwdata-bash -c '/var/www/passbolt/bin/cake passbolt install --no-admin' || wwwdata-bash -c '/var/www/passbolt/bin/cake passbolt migrate' && echo "Enjoy! ☮"
}
...
email_cron_job() {
  ...
  if [ ! -f "$cron_task" ]; then
    echo "* * * * * wwwdata-bash -c \"source /etc/environment ; /var/www/passbolt/bin/cake EmailQueue.sender\" >> /var/log/cron.log 2>&1" >> $cron_task
